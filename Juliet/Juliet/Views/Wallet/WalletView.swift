    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack {
                ...
                Spacer()
                Text(balance >= 0 ? "Active" : "Check Funds")
                    .font(.caption2.weight(.semibold))
                    .padding(.horizontal, 8).padding(.vertical, 3)
                    .background(balance >= 0 ? color.opacity(0.12) : Color.orange.opacity(0.15), in: Capsule())
                    .foregroundStyle(balance >= 0 ? color : Color.orange)
            }
            Spacer()
            Text(account)
                .font(.caption)
            ...
            Text("₹\(String(format: "%.2f", balance))")
                ...
                .foregroundStyle(balance >= 0 ? AppTheme.text : Color.orange)
        }
        .padding(14)
